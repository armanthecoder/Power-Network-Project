...
FPS = 60
R   = 40.0      # desired separation (void radius)
COMM_RANGE = 2*R     # communication radius
SPEED_MAX  = 100.0     # max robot speed (pixels per second)
PIVOT_THRESHOLD = 40  # max pivot error to accept pivot role
POS_THRESH = 5.0    # position threshold for FINDING BEST LOCAL PIVOT
K_R      = 25   # radial gain
K_THETA  = 20   # angular/orbital gain
SINK_TOUCH_DIST = COMM_RANGE   # or maybe R, you can tune this
...
SIDE = 200.0
SOURCE_POS = Vec2(WIDTH/2 - SIDE/2, HEIGHT/2 + SIDE/4)
SINK1_POS  = SOURCE_POS + Vec2(SIDE, 0.0)-(100, 200)
SINK2_POS  = SOURCE_POS + Vec2(SIDE/2.0, - (math.sqrt(3)/2.0)*SIDE)+(300, 76)
...
class Robot:
    ...
    def assign_child_to_sink(parent: Robot, child: Robot, sink_index: int):
        ...
        # for visualization: where this child *should* converge to
    ...
    def _free_behavior(self, dt: float, robots: List["Robot"]):
        ...
        if askers:
            # --- existing "somebody wants children" behavior (unchanged) ---
            ...
            # no ASK_PARENT messages → follow guidance from nearest network robot
            best_guidance = None
            best_dist = float("inf")
            ...
            for other in robots:
                ...
                g = guidance_dir_from_robot(other, robots, pivot_id=None)  # we will pass real pivot_id from main
                if g is None:
                    continue
    ...
    def _attached_behavior(self, dt: float, robots: List["Robot"]):
        ...
        can_recruit = (
            ...
            and not self.branch_done      # <<< add this
        )
...
def guidance_dir_from_robot(r: Robot, robots: List[Robot], pivot_id: Optional[int]) -> Optional[Vec2]:
    ...
    - At pivot:
        only consider children on unfinished branch; guide into that side.
    """
    if r.role != Role.NETWORK:
        return None

    # Pivot special case: steer into the unfinished branch
    ...
    if r.branch_done:
        # finished branch: guide back to parent
