...
from constants_paper_version import (
    Robot, Sink, Message, Role, NodeType,
...
def robot_step(robot: Robot, dt: float, robots: List[Robot], sinks: List[Sink], touched: List[bool]):
    ...
    if robot.role == Role.SOURCE:
        _source_behavior(robot, dt, robots, sinks, touched)
...
def _source_behavior(robot: Robot, dt: float, robots: List[Robot], sinks: List[Sink], touched: List[bool]):
    ...
    if valid_askers:
        ...
        if robot.guidance_lock_robot_id is not None and robot.guidance_lock_frames > 0:
            if 0 <= robot.guidance_lock_robot_id < len(robots):
                ...
                if guide.role == Role.NETWORK:
                    g = guidance_dir_from_robot(guide, robots, sinks, touched)
                    if g is None:
        ...
        if locked_guidance is None:
            ...
            for other in robots:
                ...
                g = guidance_dir_from_robot(other, robots, sinks, touched)
                if g is None:
                    continue
        ...
        if locked_guidance is not None:
            ...
            if locked_guide_robot is not None:
                ...
                target = locked_guide_robot.pos + guidance_dir * guide_R
                guide_speed = max(locked_guide_robot.vel.length(), SPEED_MAX)
                _move_towards_with_speed(robot, target, guide_speed, dt)
            else:
                target = robot.pos + guidance_dir * R
                _move_towards_with_speed(robot, target, SPEED_MAX, dt)

            if robot.guidance_lock_frames > 0:
                robot.guidance_lock_frames -= 1
        else:
            # gentle pull back to the source when no guidance is sensed
            source = robots[0]
            _move_towards_with_speed(robot, source.pos, SPEED_MAX, dt)
...
def _attached_behavior(robot: Robot, dt: float, robots: List[Robot], sinks: List[Sink], touched: List[bool]):
    ...
    if robot.node_type == NodeType.PIVOT:
        ...
        can_recruit = (not done) and robot.has_free_slot() and (not leaf_with_zero_sink)
        ...
        leaf_with_zero_sink = len(robot.sink_indices) == 0 and not robot.children_ids
        if done:
            can_recruit = robot.has_free_slot() and (not leaf_with_zero_sink)
        else:
            ...
            can_recruit = robot.has_free_slot() and (not all_sinks_near) and (not leaf_with_zero_sink)
